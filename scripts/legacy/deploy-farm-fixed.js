...
async function main() {
  ...
  // Fund the farm with reward tokens (transfer directly)
  console.log("\n💰 Funding farm with reward tokens...");
  const fundAmount = hre.ethers.parseEther("200"); // 200 TIK tokens for rewards
  
  const tikToken = await hre.ethers.getContractAt("LaunchToken", rewardToken);
  const transferTx = await tikToken.transfer(farmAddress, fundAmount);
  ...
  // Save farm deployment info
  const farmDeployment = {
    farm: farmAddress,
    rewardToken: rewardToken,
    rewardPerSecond: rewardPerSecond.toString(),
    pools: config.pairs.map((pair, idx) => {
      let allocPoint = 50;
      if (pair.name === "TIK-TOE") allocPoint = 100;
      if (pair.name === "TIK-TAK") allocPoint = 75;
      return {
  ...
  fs.writeFileSync(
    path.join(__dirname, "../farm-deployment.json"),
    JSON.stringify(farmDeployment, null, 2)
  );
  ...
  console.log("- Total Pools:", poolsAdded);
  console.log("\n🌾 Pools:");
  farmDeployment.pools.forEach((pool) => {
    console.log(`  ${pool.pid}. ${pool.name} - ${pool.allocPoint} points`);
  });
